...
1. **路徑與 Wire 欄位**：對外 request / response 必須完全對齊外部系統。
2. **內部命名**：Schema 屬性、Service 變數與 Domain 命名仍維持專案規範，不讓外部欄位命名流入內部層。
3. **橋接**：使用 Pydantic 的 `alias`、`validation_alias` 或 mapping adapter 轉換外部欄位與內部欄位。
