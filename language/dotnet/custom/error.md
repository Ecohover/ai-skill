...
| MUST | 系統代號透過 `ErrorDetail.RegisterSystemPrefix("{SYS}")` 一次性註冊 |
...
| 類別 | Prefix | 適用範圍 |
|------|--------|---------|
| `CommonError` | `"Common"` | 跨系統通用 |
| `AppError` | `"COM"` | 應用通用 |
| `{Domain}Error` | `"{PREFIX}"` | 特定領域 |

錯誤碼格式：`{系統代號}-{Prefix}-{Code}`，例如 `{SYS}-COM-40005`。
...
// 呼叫
throw AppError.InvalidEntityId.Exception();
```
...
```csharp
// 1. 無上下文
throw AppError.SomeError.Exception();

// 2. 有上下文物件
throw DomainError.BusinessRuleViolation.Exception(contextObject);

// 3. 帶附加資料回呼叫端（放入 response Data）
throw AppError.SomeError.Exception(errorObject: new { Invalid = items });
```
